...
For now, this is as simple as a [straight-through estimator](https://arxiv.org/abs/1308.3432) (boolean cast to float for $`x > 0`$ in the forward pass, $`HardTanh(x)`$ where $`min = -1, max = 1`$ in the backward pass) with a reconstruction of every 8 bits to a byte. 
